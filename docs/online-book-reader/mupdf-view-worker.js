...
// Import the WASM module (glue only — the module factory is invoked later,
// after the wasm binary has been preloaded, see below).
globalThis.__filename = "lib/mupdf-wasm.js"
importScripts("lib/mupdf-wasm.js")

// The emscripten glue loads the 14 MB wasm with a synchronous XMLHttpRequest,
...
fetch("lib/mupdf-wasm.wasm")
	...
	.then((wasmBinary) => {
		...
		libmupdf = function (injections) {
			return factory(Object.assign({}, injections, { wasmBinary }))
		}

		// Import the MuPDF bindings (this instantiates the wasm module).
		importScripts("lib/mupdf.js")

		start()
	})
	.catch((error) => {
		postMessage([ "ERROR", 0, { name: error.name, message: error.message, stack: error.stack } ])
	})
